...
EnvJasmine.runningFromCommandLine = (EnvJasmine.environment != "BROWSER");
...
EnvJasmine.load = (function(fileSeparator) {
    ...
    return function(path) {
        if (loaded.indexOf(path) == -1) {
            loaded.push(path);
            if (EnvJasmine.runningFromCommandLine) {
                ...
                */
                try {
                    var fileHandle = new java.io.FileWriter(EnvJasmine.libDir + "jasmine/jasmine-template.html", true);
                    var printHandle = new java.io.PrintWriter(fileHandle);

                    printHandle.println('<script src="file://' + path + '"></script>');
                    printHandle.close();
                } catch (err) {
                    print("ERROR IN ACCESSING FILE: " + err);
                }
            }
        }
    };
...
// instead:
// create html file with java.io.File
//      what we don't want in this html file:
//          spanDir.js
//          env.rhino.1.2.js
//          jasmine-rhino-reporter
//      how:
//          modify EnvJasmine.load
//              make optional second argument of load with {"includeInBrowser":false}
//              create browserLoad that builds up the HTML page by adding script tags to a string that gets opened to a file
//              will need an HTML template that includes the css file and final script calll that includes jasmine
